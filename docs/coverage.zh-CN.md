...
| UI（Playwright）端点覆盖率 | 同一记录器，由页面操作驱动 | 67/306（21.9%） | 见下方「范围」 |
...
面板、git 协议等）。因此「UI 100%」应针对**UI 可达的 API 面**，而不是全部清单。
先在代码中显式定义该子集，再用真实页面流程（不是合成探测）把它跑到 100%。
...
1. **阶段 1 —— 基建 + API 端点覆盖（已完成）。**
   ...
   掉暴露的问题。
2. **阶段 2 —— UI 端点覆盖。** 界定 UI 可达 API 子集，先在 CI 报告（不阻断），
   再补页面流程把该子集跑到 100%。
