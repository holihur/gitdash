...
在网页端（仓库 → **项目** 标签页）可以拖拽卡片在列之间、泳道之间移动。同一份数据还可用
**列表（表格）** 或 **甘特图（时间线）** 展示；卡片可带可选的 `start_date` / `due_date`
（YYYY-MM-DD）用于排期。

## 视图

项目工具栏可在同一批卡片的三种视图间切换：

- **看板** —— 列 × 泳道的拖拽网格
- **列表** —— 每张卡片的表格（列、泳道、开始、截止；逾期日期标红）
- **甘特图** —— 至少有 1 个日期的卡片按泳道分组的时间线，带“今天”标记，无日期的卡片归入“未排期”

日期在卡片弹窗中编辑（看板卡片、列表 / 甘特行上的铅笔图标）。

## 网页端使用

1. 打开仓库 → **项目** 标签页
2. 创建项目（名称 + 描述）
3. 点击项目进入：拖拽卡片跨列 / 跨泳道，点列里的 `+` 添加卡片（输入 `#12` 关联 12 号 issue，否则文本作为便签）
4. 工具栏切换 **看板 / 列表 / 甘特图**，并添加 / 删除列和泳道
5. 点卡片铅笔图标编辑便签与开始 / 截止日期
...
| GET/POST | `/{id}/cards` | 列出（附带 issue 标题与状态、日程）/ 创建卡片 `{column_id, swimlane_id?, issue_number?, note?, start_date?, due_date?}` |
| PATCH | `/{id}/cards/{card}` | 移动 `{column_id, swimlane_id, position}` 和/或编辑 `{note, start_date, due_date}`（空串清空日期） |
