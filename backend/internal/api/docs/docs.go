// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/admin/badges": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端徽章列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Badge"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端创建徽章",
                "parameters": [
                    {
                        "type": "string",
                        "description": "唯一标识（省略时按 label 生成）",
                        "name": "slug",
                        "in": "formData"
                    },
                    {
                        "type": "string",
                        "description": "名称",
                        "name": "label",
                        "in": "formData",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "描述（悬停显示）",
                        "name": "description",
                        "in": "formData"
                    },
                    {
                        "type": "string",
                        "description": "无图标时的 emoji 兜底",
                        "name": "emoji",
                        "in": "formData"
                    },
                    {
                        "type": "file",
                        "description": "图标（png/jpeg/gif/webp，≤1MB）",
                        "name": "image",
                        "in": "formData"
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Badge"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/badges/{id}": {
            "delete": {
                "tags": [
                    "admin"
                ],
                "summary": "管理端删除徽章",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端更新徽章",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "slug / label / description",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Badge"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/badges/{id}/grants": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端徽章授予列表",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.BadgeGrant"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端授予徽章",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "kind / owner / repo",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "admin"
                ],
                "summary": "管理端撤销徽章",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "目标类型 user/repo/org",
                        "name": "kind",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名 / 组织名 / 仓库所有者",
                        "name": "owner",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名（kind=repo 时必填）",
                        "name": "repo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/badges/{id}/image": {
            "post": {
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端设置徽章图标",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "file",
                        "description": "图标（png/jpeg/gif/webp，≤1MB）",
                        "name": "image",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Badge"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端删除徽章图标",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Badge"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/codesearch": {
            "get": {
                "description": "返回检索来源计数、索引重建次数/耗时/全量增量、索引仓库与文档规模、待重建仓库数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "代码搜索指标",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/ip-bans": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端 IP 黑名单列表",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.IPBan"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端新增 IP 黑名单",
                "parameters": [
                    {
                        "description": "cidr 与备注",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.ipBanReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.IPBan"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/ip-bans/{id}": {
            "delete": {
                "tags": [
                    "admin"
                ],
                "summary": "管理端删除 IP 黑名单",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "条目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/language-colors": {
            "post": {
                "description": "以 language -\u003e #RRGGBB 的形式全量替换配色覆盖；空 map 表示全部恢复默认。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "保存语言配色",
                "parameters": [
                    {
                        "description": "colors 映射",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/login": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端登录",
                "parameters": [
                    {
                        "description": "管理员用户名与密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.adminLoginReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/admin/logout": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端登出",
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/me": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "当前管理员",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/orgs": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端组织列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名模糊过滤",
                        "name": "q",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Org"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/orgs/{name}/ban": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端封禁/解封组织",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "banned",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.banReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/password": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "修改管理员密码",
                "parameters": [
                    {
                        "description": "当前密码与新密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.adminChangePasswordReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/quota": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "获取配额配置",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "保存默认配额",
                "parameters": [
                    {
                        "description": "默认配额（0 = 不限）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/store.Quota"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Quota"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/quota/{scope}/{name}": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "保存配额覆盖",
                "parameters": [
                    {
                        "type": "string",
                        "description": "user 或 org",
                        "name": "scope",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名或组织名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "覆盖配额",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/store.Quota"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "删除配额覆盖",
                "parameters": [
                    {
                        "type": "string",
                        "description": "user 或 org",
                        "name": "scope",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名或组织名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/repos": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端仓库列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "owner/name 模糊过滤",
                        "name": "q",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/repos/{owner}/{name}/ban": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端封禁/解封仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "banned",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.banReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/settings": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "获取系统设置",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "保存系统设置",
                "parameters": [
                    {
                        "description": "设置项（github/google/oidc/smtp/feedback 开关与配置）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/smtp/test": {
            "post": {
                "description": "使用当前生效的 SMTP 配置向指定邮箱发送测试邮件，用于校验配置。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "发送 SMTP 测试邮件",
                "parameters": [
                    {
                        "description": "to",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "502": {
                        "description": "Bad Gateway",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/users": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端用户列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名模糊过滤",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.User"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端创建用户",
                "parameters": [
                    {
                        "description": "用户名、密码与可选邮箱",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.adminCreateUserReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.User"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/users/{username}": {
            "delete": {
                "description": "级联删除用户的会话/密钥/PAT/OAuth/仓库等归属数据。返回 204。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端删除用户",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/users/{username}/ban": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端封禁/解封用户",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "banned",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.banReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/admin/users/{username}/reset_password": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "admin"
                ],
                "summary": "管理端重置用户密码",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "新密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.adminResetPasswordReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/announcement": {
            "get": {
                "description": "返回当前启用的全站公告（标题/正文/级别）；未启用或内容为空时 enabled=false。id 为内容指纹，客户端据此判断公告是否已被用户关闭。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "misc"
                ],
                "summary": "全站通知",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/auth/forgot-password": {
            "post": {
                "description": "根据邮箱发送重置链接。无论邮箱是否存在都返回 200（防止账户枚举）；\nSMTP 未配置或邮箱非法时同样返回 200 但不发送。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "请求密码重置",
                "parameters": [
                    {
                        "description": "email",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/login": {
            "post": {
                "description": "返回会话 token；若启用 MFA 则返回 mfa_required 与临时 mfa_token。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "登录",
                "parameters": [
                    {
                        "description": "用户名与密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.loginReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "会话 token 或 MFA 挑战",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/logout": {
            "post": {
                "description": "作废当前 token（或 cookie 会话）并清除 cookie。返回 204。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "登出",
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/auth/mfa-email/resend": {
            "post": {
                "description": "基于（未过期的）mfa_token 重新生成并发送验证码；旧验证码作废。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "重发 email MFA 登录验证码",
                "parameters": [
                    {
                        "description": "mfa_token",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "503": {
                        "description": "Service Unavailable",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/mfa-verify": {
            "post": {
                "description": "校验 TOTP 代码后签发正式会话 token。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "MFA 二次验证",
                "parameters": [
                    {
                        "description": "mfa_token 与 TOTP code",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.mfaVerifyReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/providers": {
            "get": {
                "description": "返回 github 与 oidc 的启用状态及授权 URL（未启用不暴露配置）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "列出第三方登录方式",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            }
        },
        "/auth/register": {
            "post": {
                "description": "注册成功返回 201 与会话 token。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "注册新用户",
                "parameters": [
                    {
                        "description": "用户名与密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.registerReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/reset-password": {
            "post": {
                "description": "校验邮件链接中的一次性令牌，设置新密码并撤销该用户的全部会话。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "auth"
                ],
                "summary": "重置密码",
                "parameters": [
                    {
                        "description": "token 与新密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/badges": {
            "get": {
                "description": "返回用户 / 仓库 / 组织主页上挂出的徽章（公开）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "badges"
                ],
                "summary": "目标展示的徽章",
                "parameters": [
                    {
                        "type": "string",
                        "description": "user/repo/org",
                        "name": "kind",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名 / 组织名 / 仓库所有者",
                        "name": "owner",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名（kind=repo 时必填）",
                        "name": "repo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Badge"
                            }
                        }
                    }
                }
            }
        },
        "/badges/batch": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "badges"
                ],
                "summary": "批量目标徽章",
                "parameters": [
                    {
                        "description": "kind + targets[{owner,repo}]",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "items: {\\\"owner/repo\\\": [Badge]}",
                        "schema": {
                            "type": "object"
                        }
                    }
                }
            }
        },
        "/badges/display": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "badges"
                ],
                "summary": "设置挂出的徽章",
                "parameters": [
                    {
                        "description": "kind/owner/repo/badge_ids",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/badges/owned": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "badges"
                ],
                "summary": "待挂载徽章（设置用）",
                "parameters": [
                    {
                        "type": "string",
                        "description": "user/repo/org",
                        "name": "kind",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名 / 组织名 / 仓库所有者",
                        "name": "owner",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名（kind=repo 时必填）",
                        "name": "repo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "granted / displayed",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/badges/{id}/image": {
            "get": {
                "produces": [
                    "image/png"
                ],
                "tags": [
                    "badges"
                ],
                "summary": "徽章图标",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "徽章 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/connections": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "connections"
                ],
                "summary": "列出第三方账号绑定",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/api.connectionView"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/connections/{provider}": {
            "delete": {
                "tags": [
                    "connections"
                ],
                "summary": "解除第三方账号绑定",
                "parameters": [
                    {
                        "type": "string",
                        "description": "provider",
                        "name": "provider",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/connections/{provider}/callback": {
            "get": {
                "tags": [
                    "connections"
                ],
                "summary": "第三方账号绑定回调",
                "parameters": [
                    {
                        "type": "string",
                        "description": "provider",
                        "name": "provider",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "302": {
                        "description": "Found"
                    }
                }
            }
        },
        "/connections/{provider}/repos": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "connections"
                ],
                "summary": "列出远程仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "provider",
                        "name": "provider",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/api.forgeRepo"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "502": {
                        "description": "Bad Gateway",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/connections/{provider}/start": {
            "get": {
                "tags": [
                    "connections"
                ],
                "summary": "开始绑定第三方账号",
                "parameters": [
                    {
                        "type": "string",
                        "description": "github|gitlab|gitea|bitbucket",
                        "name": "provider",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "302": {
                        "description": "Found"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/explore/repos": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "探索公开仓库",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "关键词（匹配所有者/名称/描述）",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "按标签（topic）过滤",
                        "name": "topic",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/feedback": {
            "get": {
                "description": "返回反馈组件是否已由管理员启用并完成配置。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "feedback"
                ],
                "summary": "反馈功能状态",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                }
            },
            "post": {
                "description": "使用管理员配置的仓库地址与令牌，在对应仓库创建 Issue；若目标指向本实例仓库则直接创建本地 Issue（无需令牌）。登录用户会附带身份信息。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "feedback"
                ],
                "summary": "提交反馈",
                "parameters": [
                    {
                        "description": "反馈内容",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.feedbackBodyReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "502": {
                        "description": "Bad Gateway",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/gpg": {
            "post": {
                "description": "解析 armored 公钥并注册。返回 201 与新键。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "添加 GPG 公钥",
                "parameters": [
                    {
                        "description": "armored 公钥",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addGPGKeyReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.GPGKey"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/gpg/{id}": {
            "delete": {
                "description": "按 id 删除当前用户的 GPG 公钥。返回 204。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "删除 GPG 公钥",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "GPG key id",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/hooks/incoming/{owner}/{repo}": {
            "post": {
                "description": "凭仓库入站 webhook token（X-Gitdash-Token 头或 ?token=）创建 issue。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "通过入站 webhook 创建 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "repo",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题与正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createIssueReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/imports": {
            "post": {
                "description": "支持 http(s)/ssh/git 地址；URL 校验失败返回 400。导入异步执行，成功返回 202，通过 GET repo 的 import_status 轮询进度。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "导入仓库",
                "parameters": [
                    {
                        "description": "url、name（可选）、namespace（可选）、private（可选）、private_key（可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.importRepoReq"
                        }
                    }
                ],
                "responses": {
                    "202": {
                        "description": "Accepted",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/imports/batch": {
            "post": {
                "description": "使用已绑定的第三方账号，把选中的远程仓库批量创建并异步导入。单个仓库失败不影响其余。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "connections"
                ],
                "summary": "批量导入仓库",
                "parameters": [
                    {
                        "description": "provider 与 full_name 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.batchImportReq"
                        }
                    }
                ],
                "responses": {
                    "202": {
                        "description": "Accepted",
                        "schema": {
                            "$ref": "#/definitions/api.batchImportResult"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/inbox": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inbox"
                ],
                "summary": "列出收件箱通知",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "每页数量（默认 50，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Notification"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/inbox/read": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inbox"
                ],
                "summary": "全部标记已读",
                "responses": {
                    "200": {
                        "description": "ok",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/inbox/read/{id}": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inbox"
                ],
                "summary": "标记通知已读",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "通知 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "ok",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/inbox/unread": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "inbox"
                ],
                "summary": "未读通知数量",
                "responses": {
                    "200": {
                        "description": "count",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/inbox/{id}": {
            "delete": {
                "tags": [
                    "inbox"
                ],
                "summary": "删除通知",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "通知 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/instance": {
            "get": {
                "description": "返回版本与 SSH 端口（前端 clone 地址展示用）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "misc"
                ],
                "summary": "实例信息",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object"
                        }
                    }
                }
            }
        },
        "/keys": {
            "get": {
                "description": "返回当前用户注册的所有 SSH 公钥。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "列出 SSH 公钥",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.SSHKey"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "校验并规范化公钥后注册。返回 201 与新键。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "添加 SSH 公钥",
                "parameters": [
                    {
                        "description": "名称与公钥内容",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createKeyReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.SSHKey"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/keys/{id}": {
            "delete": {
                "description": "按 id 删除当前用户的 SSH 公钥。返回 204。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "删除 SSH 公钥",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "SSH key id",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/languages": {
            "get": {
                "description": "返回内置支持的语言、每种语言的默认配色，以及管理端配置的颜色覆盖（language -\u003e #RRGGBB）。公开接口。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "misc"
                ],
                "summary": "语言列表与配色",
                "responses": {
                    "200": {
                        "description": "languages、defaults 与 colors",
                        "schema": {
                            "type": "object"
                        }
                    }
                }
            }
        },
        "/mail/inbound": {
            "post": {
                "description": "邮件提供商/MTA 管道调用；凭 X-Gitdash-Mail-Secret 头鉴权，\n从收件地址 reply+\u003ctoken\u003e@domain 还原仓库与 issue/PR，\n清洗正文后以 token 绑定用户身份创建评论。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "mail"
                ],
                "summary": "入站邮件（reply-by-email）",
                "parameters": [
                    {
                        "description": "收件人/发件人/主题/正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.mailInboundReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/me": {
            "get": {
                "description": "返回当前会话用户的用户名、邮箱、创建时间与 MFA 状态。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "当前用户信息",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "description": "校验密码（启用 MFA 时还需二次验证码）后，永久删除当前账号及其全部数据。返回 204。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "注销账号",
                "parameters": [
                    {
                        "description": "当前密码与（启用 MFA 时的）验证码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.deleteAccountReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/avatar": {
            "post": {
                "description": "multipart 字段 avatar；支持 png/jpeg/gif/webp，最大 2MB。",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "上传头像",
                "parameters": [
                    {
                        "type": "file",
                        "description": "头像图片",
                        "name": "avatar",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "删除头像",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/byok": {
            "get": {
                "description": "返回当前用户配置的全部 BYOK（bring your own key）密钥，不含明文。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "byok"
                ],
                "summary": "列出 BYOK 密钥",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.ByokKey"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "byok"
                ],
                "summary": "创建 BYOK 密钥",
                "parameters": [
                    {
                        "description": "name/provider/api_key/base_url/model",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.byokReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.ByokKey"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/byok/test": {
            "post": {
                "description": "向 Anthropic 兼容端点发送一次最小请求，返回是否连通及错误信息。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "byok"
                ],
                "summary": "测试 BYOK 连接",
                "parameters": [
                    {
                        "description": "provider/base_url/model/api_key(可留空回退到 id)",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.testByokReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/byok/{id}": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "byok"
                ],
                "summary": "更新 BYOK 密钥",
                "parameters": [
                    {
                        "description": "name/provider/api_key(留空保留)/base_url/model",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.byokReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.ByokKey"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "byok"
                ],
                "summary": "删除 BYOK 密钥",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/cover": {
            "post": {
                "description": "multipart 字段 cover；支持 png/jpeg/gif/webp，最大 5MB。",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "上传用户封面",
                "parameters": [
                    {
                        "type": "file",
                        "description": "封面图片",
                        "name": "cover",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "删除用户封面",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/email/resend": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "重发验证邮件",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/email/verify": {
            "post": {
                "description": "用验证令牌完成邮箱验证（令牌来自验证邮件链接）。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "验证邮箱",
                "parameters": [
                    {
                        "description": "token",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/export": {
            "get": {
                "description": "以 JSON 附件形式下载本账号在实例上的个人数据与自产内容。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "导出个人数据",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.UserExport"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa": {
            "get": {
                "description": "返回是否已启用；若存在待激活的 secret，则附带 pending_secret 与 otpauth_url。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "MFA 状态",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/activate": {
            "post": {
                "description": "校验 TOTP 代码后启用 MFA。返回 204。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "激活 MFA",
                "parameters": [
                    {
                        "description": "TOTP code",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.mfaActivateReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/disable": {
            "post": {
                "description": "需提供当前密码与有效 TOTP 代码。返回 204。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "关闭 MFA",
                "parameters": [
                    {
                        "description": "当前密码与 TOTP code",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.mfaDisableReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/email/activate": {
            "post": {
                "description": "校验邮箱激活码后启用 MFA（方式为 email）。返回 204。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "激活 email MFA",
                "parameters": [
                    {
                        "description": "code",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/email/enroll": {
            "post": {
                "description": "要求已验证邮箱；发送 6 位激活码（10 分钟有效）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "绑定 email MFA",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/email/send": {
            "post": {
                "description": "向已验证邮箱发送验证码，用于 /me/mfa/disable 校验。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "发送 email MFA 验证码",
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/mfa/enroll": {
            "post": {
                "description": "返回 secret 与 otpauth URL，供认证器扫码；尚未激活。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "开始 MFA 绑定",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/password": {
            "post": {
                "description": "校验当前密码后更新，并撤销其它会话（仅保留当前会话）。返回 204。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "修改密码",
                "parameters": [
                    {
                        "description": "当前密码与新密码",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.changePasswordReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/me/profile": {
            "post": {
                "description": "更新当前用户邮箱与邮件通知开关；邮箱空串表示清除。返回 200 与更新后的资料。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "更新个人资料",
                "parameters": [
                    {
                        "description": "邮箱（可为空串）与 notify_email",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateProfileReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "列出我的组织",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "创建组织",
                "parameters": [
                    {
                        "description": "组织名与显示名",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createOrgReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "获取组织",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "删除组织",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "修改组织信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Org"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/cover": {
            "get": {
                "description": "返回组织主页顶部横幅图片；未设置封面返回 404。",
                "produces": [
                    "image/png"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "组织封面",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "multipart 字段 cover；支持 png/jpeg/gif/webp，最大 5MB；仅组织 owner。",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "上传组织封面",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "file",
                        "description": "封面图片",
                        "name": "cover",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "删除组织封面",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/follow": {
            "post": {
                "description": "幂等。返回关注状态与粉丝数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "关注组织",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "description": "幂等。返回关注状态与粉丝数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "取消关注组织",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/followers": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "组织粉丝列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.UserSummary"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/members": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "列出组织成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "添加组织成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "用户名与角色（member/owner）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addOrgMemberReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/members/{username}": {
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "移除组织成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "成员用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/profile": {
            "get": {
                "description": "返回组织公开资料、粉丝数、当前用户是否已关注、成员列表，以及可见仓库（非成员仅公开仓库）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "组织主页",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/repos": {
            "get": {
                "description": "返回组织仓库列表与当前用户在组织中的角色。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "组织仓库列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "role 与 repos",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/teams": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "列出组织团队",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.OrgTeam"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "创建组织团队",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "name",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.OrgTeam"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/teams/{id}": {
            "delete": {
                "tags": [
                    "orgs"
                ],
                "summary": "删除组织团队",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/teams/{id}/members": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "团队详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "orgs"
                ],
                "summary": "添加团队成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "username",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/orgs/{org}/teams/{id}/members/{username}": {
            "delete": {
                "tags": [
                    "orgs"
                ],
                "summary": "移除团队成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "组织名",
                        "name": "org",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/package-files/{type}/{owner}/{name}": {
            "get": {
                "description": "不带 filename 时列出包的全部版本制品；带 filename 时：",
                "tags": [
                    "packages"
                ],
                "summary": "浏览包文件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "包类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "版本",
                        "name": "version",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "制品文件名",
                        "name": "filename",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "=1 列出归档条目",
                        "name": "entries",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "归档内条目名",
                        "name": "entry",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "=1 作为附件下载",
                        "name": "download",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    }
                }
            }
        },
        "/packages/cargo/{owner}/api/v1/crates/new": {
            "put": {
                "tags": [
                    "packages"
                ],
                "summary": "cargo 发布",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    }
                }
            }
        },
        "/packages/cargo/{owner}/api/v1/crates/{crate}/{version}/yank": {
            "delete": {
                "tags": [
                    "packages"
                ],
                "summary": "cargo yank / unyank",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "crate 名",
                        "name": "crate",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "版本",
                        "name": "version",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    }
                }
            }
        },
        "/packages/cargo/{owner}/config.json": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "cargo config",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            }
        },
        "/packages/cargo/{owner}/index/{rest}": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "cargo 索引",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "稀疏索引路径（最后一段为 crate 名）",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            }
        },
        "/packages/composer/{owner}/packages.json": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "composer 元数据",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            }
        },
        "/packages/composer/{owner}/{vendor}/{name}": {
            "put": {
                "tags": [
                    "packages"
                ],
                "summary": "composer 上传",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "vendor",
                        "name": "vendor",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "版本",
                        "name": "version",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created"
                    }
                }
            }
        },
        "/packages/go/{owner}/{rest}": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "go GOPROXY",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "{module}/@v/{file}",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            },
            "put": {
                "tags": [
                    "packages"
                ],
                "summary": "go 上传",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "{module}/@v/{version}.zip",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created"
                    }
                }
            }
        },
        "/packages/maven/{owner}/{rest}": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "maven 下载",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "{group}/{artifact}/{version}/{filename}",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            },
            "put": {
                "tags": [
                    "packages"
                ],
                "summary": "maven 上传",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "{group}/{artifact}/{version}/{filename}",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created"
                    }
                }
            }
        },
        "/packages/npm/{owner}/{rest}": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "npm 元数据 / tarball",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名 或 包名/-/文件名 或 dist-tags 路径",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            },
            "put": {
                "tags": [
                    "packages"
                ],
                "summary": "npm 发布 / dist-tag",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名（可含 @scope/）或 dist-tag 路径",
                        "name": "rest",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created"
                    }
                }
            }
        },
        "/packages/pypi/{owner}/": {
            "post": {
                "tags": [
                    "packages"
                ],
                "summary": "pypi 上传",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    }
                }
            }
        },
        "/packages/pypi/{owner}/pypi/{name}/json": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "pypi JSON API",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            }
        },
        "/packages/pypi/{owner}/simple": {
            "get": {
                "tags": [
                    "packages"
                ],
                "summary": "pypi simple index",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {}
            }
        },
        "/packages/rubygems/{owner}/api/v1/gems": {
            "post": {
                "tags": [
                    "packages"
                ],
                "summary": "rubygems 发布",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created"
                    }
                }
            }
        },
        "/packages/{owner}/audit": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "packages"
                ],
                "summary": "包审计记录",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包类型",
                        "name": "type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.PackageAudit"
                            }
                        }
                    }
                }
            }
        },
        "/packages/{owner}/{type}": {
            "get": {
                "description": "列出 owner 命名空间下的包（type 可选过滤：npm/composer/pypi/rubygems/go/cargo/maven/docker）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "packages"
                ],
                "summary": "列出包",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包类型",
                        "name": "type",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "按包名模糊搜索",
                        "name": "q",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Package"
                            }
                        }
                    }
                }
            }
        },
        "/packages/{type}/{owner}/{name}": {
            "delete": {
                "tags": [
                    "packages"
                ],
                "summary": "删除包",
                "parameters": [
                    {
                        "type": "string",
                        "description": "包类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                }
            },
            "patch": {
                "tags": [
                    "packages"
                ],
                "summary": "设置包可见性",
                "parameters": [
                    {
                        "type": "string",
                        "description": "包类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "用户或组织",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "包名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK"
                    }
                }
            }
        },
        "/repos": {
            "get": {
                "description": "返回当前用户可访问的全部仓库（含 star/watch 状态）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出可访问仓库",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "创建成功返回 201。namespace 可选，指定组织名可将仓库建到组织下。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "创建仓库",
                "parameters": [
                    {
                        "description": "仓库名、描述、模板（readme：README.md + .gitdash.yml）、是否私有、组织命名空间",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createRepoReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}": {
            "get": {
                "description": "返回仓库信息（含 fork/导入来源、star/watch 状态与当前用户角色）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "获取仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/blame": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "文件 blame",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.Blame"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/blob": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取文件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.Blob"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/branches": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出分支",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/gitsvc.Branch"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/comments/{id}": {
            "delete": {
                "tags": [
                    "comments"
                ],
                "summary": "删除评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "评论 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "编辑评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "评论 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/commits": {
            "get": {
                "description": "返回提交列表，含 GPG 验证结果（对已注册公钥）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "提交历史",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "返回条数上限（默认 30，最大 100）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "跳过条数（用于“加载更多”）",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/api.commitResp"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/gc": {
            "post": {
                "description": "运行 ` + "`" + `git gc` + "`" + ` 打包松散对象并回收磁盘空间，仅仓库所有者可执行。返回回收前后的字节数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "仓库垃圾回收",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.GCResult"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/issues": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "列出 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "关键词（标题/正文/作者）",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态过滤：open 或 closed（空 = 全部）",
                        "name": "state",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "里程碑过滤：里程碑 id 或 none（未指派，空 = 全部）",
                        "name": "milestone",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Issue"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "创建 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题与正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createIssueReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/issues/counts": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "Issue 状态计数",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "标签 id 或 none",
                        "name": "label",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "里程碑 id 或 none",
                        "name": "milestone",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "负责人用户名、me 或 none",
                        "name": "assignee",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "integer"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/issues/{number}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "获取 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "issues"
                ],
                "summary": "删除 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "description": "局部更新标题、正文、状态与置顶；至少提供一个字段。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "编辑 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题 / 正文 / 状态（均可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateIssueReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/issues/{number}/comments": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "列出评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Comment"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "需要仓库写权限；成功返回 201 并向关注者推送通知。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "发表评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addCommentReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/pulls/{number}/comments": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "列出评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Comment"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "需要仓库写权限；成功返回 201 并向关注者推送通知。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "发表评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addCommentReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/raw": {
            "get": {
                "description": "以正确的 Content-Type 返回文件原始字节（图片 / PDF 等可直接内联查看）。",
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "原始文件内容",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/releases": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Release"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "tag 必须已存在（不存在返回 400 tag_not_found）；同 tag 重复创建返回 409。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "创建 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "tag_name、name（可选）与 body（可选，markdown）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createReleaseReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Release"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/releases/{tag}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Release"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "删除 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/search": {
            "get": {
                "description": "固定字符串全文搜索（跳过二进制文件），返回 {path, line, text} 列表。\n以空格分隔的多个关键词按 AND 语义（命中行需同时包含全部关键词）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "代码搜索",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "搜索字符串",
                        "name": "q",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit（默认默认分支）",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "返回条数上限（默认 50，最大 200）",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/gitsvc.SearchHit"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/repos/{name}/tree": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "浏览目录树",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit（默认默认分支）",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "目录路径（默认根目录）",
                        "name": "path",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "path、entries 与 truncated",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/runner/register": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "runners"
                ],
                "summary": "注册 runner",
                "parameters": [
                    {
                        "description": "name/labels/token",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.registerRunnerReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/runners": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "runners"
                ],
                "summary": "列出 runner",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Runner"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/runners/registration-token": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "runners"
                ],
                "summary": "签发 runner 注册 token",
                "parameters": [
                    {
                        "description": "scope: user|org（org 需 body.org）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createRunnerTokenReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.RunnerToken"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/search": {
            "get": {
                "description": "按 q 模糊搜索公开仓库、issue（公开仓库 + 自己仓库）与用户/组织，返回 {repos, issues, users}。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "search"
                ],
                "summary": "全局搜索",
                "parameters": [
                    {
                        "type": "string",
                        "description": "搜索字符串",
                        "name": "q",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每类返回条数上限（默认 20，最大 50）",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/search/code": {
            "get": {
                "description": "在当前用户可访问的仓库 + 公开仓库上做固定字符串搜索。\nq 支持内联限定符 repo:owner/name、lang:go、path:src/、symbol:Foo。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "search"
                ],
                "summary": "全局代码搜索",
                "parameters": [
                    {
                        "type": "string",
                        "description": "关键词与内联限定符",
                        "name": "q",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库限定（owner/name 或 name）",
                        "name": "repo",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "语言限定",
                        "name": "lang",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "路径前缀限定",
                        "name": "path",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "符号（单词边界匹配）",
                        "name": "symbol",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "results / truncated / repos_searched",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/starred": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出收藏仓库",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/templates": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出可访问的模版仓库",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/tokens": {
            "get": {
                "description": "返回当前用户创建的所有 PAT（不含明文 token）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tokens"
                ],
                "summary": "列出个人访问令牌",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.PAT"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "校验 name/scopes/cidrs/expires_at 后生成新 PAT，明文 token 只在本次响应中出现。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tokens"
                ],
                "summary": "创建个人访问令牌",
                "parameters": [
                    {
                        "description": "名称、授权范围、来源 IP 白名单与过期时间",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createPATReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.CreatedPAT"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/tokens/{id}": {
            "delete": {
                "description": "按 id 删除当前用户的 PAT，令牌立即失效。返回 204。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "tokens"
                ],
                "summary": "删除个人访问令牌",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "PAT id",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/topics": {
            "get": {
                "description": "返回公开仓库的 topic 及使用数，供 Explore 筛选。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出仓库标签",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.TopicCount"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}": {
            "get": {
                "description": "返回仓库信息（含 fork/导入来源、star/watch 状态与当前用户角色）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "获取仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/access": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "仓库权限审计",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.AccessEntry"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/blame": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "文件 blame",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.Blame"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/blob": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取文件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.Blob"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/branch-protections": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "branch-protection"
                ],
                "summary": "列出分支保护",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.BranchProtection"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/branch-protections/{branch}": {
            "put": {
                "description": "需要 owner 权限；min_approvals\u003e0 时合并该分支的 PR 需要对应数量的 approve。",
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "branch-protection"
                ],
                "summary": "设置分支保护",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支名",
                        "name": "branch",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "保护规则",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setBranchProtectionReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.BranchProtection"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "branch-protection"
                ],
                "summary": "移除分支保护",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支名",
                        "name": "branch",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/branches": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出分支",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/gitsvc.Branch"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/collabs": {
            "get": {
                "description": "仅仓库所有者可查看。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出协作者",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Collab"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "permission 为 read / triage / write / maintain / admin；仅仓库所有者或 admin 协作者可操作。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "添加协作者",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "username 与 permission",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addCollabReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Collab"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/collabs/{username}": {
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "移除协作者",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "协作者用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/comments/{id}": {
            "delete": {
                "tags": [
                    "comments"
                ],
                "summary": "删除评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "评论 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "编辑评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "评论 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/commit-rules": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取提交身份规则",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "enabled/name_pattern/email_pattern",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "更新提交身份规则",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "name_pattern/email_pattern",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "enabled/name_pattern/email_pattern",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/commits": {
            "get": {
                "description": "返回提交列表，含 GPG 验证结果（对已注册公钥）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "提交历史",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "返回条数上限（默认 30，最大 100）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "跳过条数（用于“加载更多”）",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/api.commitResp"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "支持批量文件变更（create/update/delete/delete_tree/move），总内容不超过 2MB。move 通过 from 指定原路径。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "创建提交",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "branch（默认 main）、message 与 changes",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.writeCommitReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "sha、branch 与 message",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/commits/{sha}": {
            "get": {
                "description": "返回单个提交的 sha / author / date / message / parents / refs，供 blame 深链跳转。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "提交详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "commit SHA",
                        "name": "sha",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.Commit"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/commits/{sha}/diff": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "提交 diff",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "commit SHA",
                        "name": "sha",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "files 与 patch",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/commits/{sha}/revert": {
            "post": {
                "description": "在 branch 上创建撤销 sha 变更的新提交；merge 提交按第一父提交撤销；冲突时返回 409。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "撤销提交",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "commit SHA",
                        "name": "sha",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "branch 与可选 message",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.revertCommitReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "sha、branch 与 message",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/compare": {
            "get": {
                "description": "返回 base..head 之间的文件统计与统一 diff；base / head 可为分支名、标签或 commit SHA。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "比较引用",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "基准引用",
                        "name": "base",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "目标引用",
                        "name": "head",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "base、head、base_sha、head_sha、files 与 patch",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/copilots": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "列出 copilot 会话",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.CopilotSession"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "创建 copilot 会话",
                "parameters": [
                    {
                        "description": "byok_id/prompt",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.copilotCreateReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.CopilotSession"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/copilots/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "获取 copilot 会话",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.CopilotSession"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "删除 copilot 会话",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/copilots/{id}/chat": {
            "get": {
                "tags": [
                    "copilot"
                ],
                "summary": "copilot 双向聊天（WebSocket）",
                "responses": {},
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/copilots/{id}/messages": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "获取 copilot 对话历史",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/copilot.ChatMessage"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/copilots/{id}/stop": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "copilot"
                ],
                "summary": "取消 copilot 当前轮次",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.CopilotSession"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/default-branch": {
            "post": {
                "description": "仅仓库所有者可设置，且目标分支必须已存在。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置默认分支",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "branch",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoDefaultBranchReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/deploy-keys": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出部署密钥",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.DeployKey"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "新增部署密钥",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "title/key/read_only",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.DeployKey"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/deploy-keys/{id}": {
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "删除部署密钥",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "部署密钥 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/description": {
            "post": {
                "description": "仅仓库所有者可修改；描述可置空。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "修改仓库描述",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "description",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoDescriptionReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/env": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "列出仓库环境变量",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "设置仓库环境变量",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "key 与 value",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoEnvVarReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/env/{key}": {
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "删除仓库环境变量",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "环境变量名",
                        "name": "key",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/fork": {
            "post": {
                "description": "fork 保持源仓库可见性，创建成功返回 201。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "fork 仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "源仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "源仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "目标名称与组织命名空间（可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.forkRepoReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "403": {
                        "description": "Forbidden",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/gc": {
            "post": {
                "description": "运行 ` + "`" + `git gc` + "`" + ` 打包松散对象并回收磁盘空间，仅仓库所有者可执行。返回回收前后的字节数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "仓库垃圾回收",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/gitsvc.GCResult"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/incoming-webhook": {
            "get": {
                "description": "返回该仓库是否已配置入站 webhook（不返回 token 明文）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "查看入站 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "enabled / created_at / last_used_at / path",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "生成（或轮换）用于创建 issue 的入站 webhook token；明文 token 仅此一次返回。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "创建/轮换入站 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "token / path / created_at",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "删除入站 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "列出 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "关键词（标题/正文/作者）",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态过滤：open 或 closed（空 = 全部）",
                        "name": "state",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "里程碑过滤：里程碑 id 或 none（未指派，空 = 全部）",
                        "name": "milestone",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Issue"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "创建 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题与正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createIssueReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues-enabled": {
            "post": {
                "description": "仅仓库所有者可设置。关闭后该仓库不可再创建 issue。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置 issue 开关",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "has_issues",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoIssuesReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/counts": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "Issue 状态计数",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "q",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "标签 id 或 none",
                        "name": "label",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "里程碑 id 或 none",
                        "name": "milestone",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "负责人用户名、me 或 none",
                        "name": "assignee",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "integer"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "获取 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "issues"
                ],
                "summary": "删除 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "description": "局部更新标题、正文、状态与置顶；至少提供一个字段。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "编辑 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题 / 正文 / 状态（均可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateIssueReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/assignees": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "设置 Issue 负责人",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "负责人用户名列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/comments": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "列出评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Comment"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "需要仓库写权限；成功返回 201 并向关注者推送通知。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "发表评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addCommentReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/events": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "Issue 时间线",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.IssueEvent"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/labels": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "设置 Issue 标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标签 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setIssueLabelsReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/milestone": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "设置 Issue 里程碑",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "里程碑 ID（0 = 清除）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setIssueMilestoneReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Issue"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/issues/{number}/subscribe": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "订阅 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "取消订阅 Issue",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "boolean"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/labels": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "列出标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Label"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "创建标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "名称与颜色",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createLabelReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Label"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/labels/{id}": {
            "delete": {
                "tags": [
                    "issues"
                ],
                "summary": "删除标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "标签 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "更新标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "标签 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "名称与颜色",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateLabelReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Label"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/member-role": {
            "post": {
                "description": "仅组织仓库可用，需仓库 admin；空字符串表示清除覆盖、回退组织默认。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置组织成员默认角色",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "role",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoMemberRoleReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/milestones": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "列出里程碑",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Milestone"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "创建里程碑",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题与描述",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createMilestoneReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Milestone"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/milestones/{id}": {
            "delete": {
                "tags": [
                    "issues"
                ],
                "summary": "删除里程碑",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "里程碑 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "issues"
                ],
                "summary": "更新里程碑",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "里程碑 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题、描述与状态",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateMilestoneReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Milestone"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/mirror": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "查看镜像配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "url 与 created_at",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置镜像",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "url 与 private_key（可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setMirrorReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "url、status 与 created_at",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "删除镜像",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/mirror/sync": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "同步镜像",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "202": {
                        "description": "status=queued",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pages": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取仓库 Pages 配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "enabled/branch/dir/url",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "更新仓库 Pages 配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "enabled/branch/dir",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "enabled/branch/dir/url",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/patches": {
            "post": {
                "description": "提交 ` + "`" + `git format-patch` + "`" + ` / ` + "`" + `git send-email` + "`" + ` 风格的 mbox（text/plain）。\n服务端 ` + "`" + `git am --3way` + "`" + ` 到新分支 patches/\u003cts\u003e 后自动开 PR。",
                "consumes": [
                    "text/plain"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "通过补丁邮件创建 PR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "目标分支（默认仓库默认分支）",
                        "name": "target",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "PR 标题（默认取补丁首个 Subject）",
                        "name": "title",
                        "in": "query"
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "获取流水线配置",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "设置流水线开关",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "{enabled: bool}",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setPipelineReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/dispatch": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "外部 dispatch 触发流水线",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "可选 ref/sha/file/inputs",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/api.createPipelineRunReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/graph": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "流水线可视化图",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支或 tag（默认仓库默认分支）",
                        "name": "ref",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/pipeline.Graph"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "列出流水线运行",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "数量上限",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "手动触发流水线",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "可选 ref/sha/file/delay/inputs",
                        "name": "body",
                        "in": "body",
                        "schema": {
                            "$ref": "#/definitions/api.createPipelineRunReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "429": {
                        "description": "Too Many Requests",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "获取流水线运行详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs/{id}/artifacts": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "列出运行产物",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs/{id}/artifacts/download": {
            "get": {
                "produces": [
                    "application/gzip"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "下载运行产物",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs/{id}/cancel": {
            "post": {
                "tags": [
                    "pipeline"
                ],
                "summary": "取消流水线运行",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pipeline/runs/{id}/rerun": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "重跑流水线运行",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "既有运行 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "列出看板项目",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Project"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}": {
            "delete": {
                "tags": [
                    "projects"
                ],
                "summary": "删除看板项目",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "更新看板项目",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "名称与描述",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateProjectReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Project"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/board": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "获取看板数据",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/api.boardResp"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/cards": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "列出看板卡片",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.ProjectCard"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "创建看板卡片",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "列 / 泳道 / issue / 文本",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createProjectCardReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.ProjectCard"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/cards/{card}": {
            "delete": {
                "tags": [
                    "projects"
                ],
                "summary": "删除看板卡片",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "卡片 ID",
                        "name": "card",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "更新看板卡片",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "卡片 ID",
                        "name": "card",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "移动 / 编辑内容",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateProjectCardReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.ProjectCard"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/cards/{card}/assignees": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "设置看板卡片负责人",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "卡片 ID",
                        "name": "card",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "负责人用户名列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/cards/{card}/labels": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "设置看板卡片标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "卡片 ID",
                        "name": "card",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标签 ID 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/columns": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "列出看板列",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.ProjectColumn"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "创建看板列",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "列名",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createProjectColumnReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.ProjectColumn"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/columns/{cid}": {
            "delete": {
                "tags": [
                    "projects"
                ],
                "summary": "删除看板列",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "列 ID",
                        "name": "cid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "卡片迁移目标列 ID（省略则一并删除卡片）",
                        "name": "move_to",
                        "in": "query"
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "更新看板列",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "列 ID",
                        "name": "cid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "列名与顺序",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateProjectColumnReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/swimlanes": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "列出看板泳道",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.ProjectSwimlane"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "创建看板泳道",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "泳道名",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createProjectSwimlaneReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.ProjectSwimlane"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/projects/{id}/swimlanes/{lid}": {
            "delete": {
                "tags": [
                    "projects"
                ],
                "summary": "删除看板泳道",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "泳道 ID",
                        "name": "lid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "patch": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "projects"
                ],
                "summary": "更新看板泳道",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "项目 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "泳道 ID",
                        "name": "lid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "泳道名与顺序",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.updateProjectSwimlaneReq"
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "列出 PR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "状态过滤（open/closed/merged）",
                        "name": "state",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.PullRequest"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "创建 PR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "标题、正文、源分支与目标分支",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createPullReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "获取 PR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/auto-merge": {
            "post": {
                "description": "需要仓库写权限；开启后当合并门禁满足时由服务端自动按指定方式合并。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "设置 PR 自动合并",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "enabled 与可选 method",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/codeowners": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "获取 PR 的 CODEOWNERS 状态",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "files、owners、approved、missing、satisfied",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/comments": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "列出评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Comment"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "需要仓库写权限；成功返回 201 并向关注者推送通知。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "comments"
                ],
                "summary": "发表评论",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（owner 路由时）",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Issue/PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "评论正文",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.addCommentReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Comment"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/comments/{id}/apply": {
            "post": {
                "description": "需要仓库写权限；在 PR 源分支上创建提交，替换被评论行，并记录应用的提交 SHA。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "应用代码建议",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "评论 ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "sha 与更新后的评论",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/diff": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "获取 PR diff",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "files、patch、base_sha、head_sha",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/draft": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "切换 PR 草稿状态",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "draft: true/false",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/merge": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "合并 PR",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "合并方式（fast-forward/merge/squash/rebase）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.mergePullReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/merge-queue": {
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "移出合并队列",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/reviews": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "列出 PR reviews",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "reviews 数组与 summary {approvals, request_changes}",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "需要仓库写权限；同一 reviewer 重复提交插入新行（保留历史）。成功返回 201 并向关注者推送通知与 webhook 事件。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "提交 PR review",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "review 状态与说明",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createReviewReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.PullReview"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/pulls/{number}/state": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pulls"
                ],
                "summary": "修改 PR 状态",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "PR 编号",
                        "name": "number",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "状态（open/closed）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setPullStateReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.PullRequest"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/raw": {
            "get": {
                "description": "以正确的 Content-Type 返回文件原始字节（图片 / PDF 等可直接内联查看）。",
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "原始文件内容",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件路径",
                        "name": "path",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/refs": {
            "post": {
                "description": "type 必须为 branch 或 tag；from 缺省为 HEAD。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "创建分支/标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "type、name、from（可选）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createRefReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "type、name 与 sha",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/refs/{kind}/{refname}": {
            "delete": {
                "description": "不能删除默认（HEAD）分支。",
                "tags": [
                    "repos"
                ],
                "summary": "删除分支/标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "类型：branches 或 tags",
                        "name": "kind",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签名",
                        "name": "refname",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/refs/{kind}/{refname}/note": {
            "put": {
                "description": "note 为空时清除备注；备注与 git refs 分离存储，不影响仓库内容。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置引用备注",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "类型：branch 或 tag",
                        "name": "kind",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签名",
                        "name": "refname",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "note 备注内容",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRefNoteReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "kind、name 与 note",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/releases": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Release"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "tag 必须已存在（不存在返回 400 tag_not_found）；同 tag 重复创建返回 409。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "创建 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "tag_name、name（可选）与 body（可选，markdown）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createReleaseReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.Release"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/releases/{tag}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "读取 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Release"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "删除 release",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/releases/{tag}/assets": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出 release 附件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.ReleaseAsset"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "description": "单文件最大 10MB，每个 release 最多 10 个附件；同 release 内文件名重复返回 409。",
                "consumes": [
                    "multipart/form-data"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "上传 release 附件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "file",
                        "description": "附件文件",
                        "name": "file",
                        "in": "formData",
                        "required": true
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/store.ReleaseAsset"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/releases/{tag}/assets/{filename}": {
            "get": {
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "下载 release 附件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "附件文件名",
                        "name": "filename",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "删除 release 附件",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "tag 名",
                        "name": "tag",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "附件文件名",
                        "name": "filename",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content",
                        "schema": {
                            "type": "string"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/search": {
            "get": {
                "description": "固定字符串全文搜索（跳过二进制文件），返回 {path, line, text} 列表。\n以空格分隔的多个关键词按 AND 语义（命中行需同时包含全部关键词）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "代码搜索",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "搜索字符串",
                        "name": "q",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit（默认默认分支）",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "返回条数上限（默认 50，最大 200）",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/gitsvc.SearchHit"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/secrets": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "列出仓库 secrets",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "设置仓库 secret",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "name 与 value",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoSecretReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/secrets/{secret}": {
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "pipeline"
                ],
                "summary": "删除仓库 secret",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "secret 名",
                        "name": "secret",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/star": {
            "put": {
                "description": "幂等；返回当前收藏状态与 star 数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "收藏仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "starred 与 stars",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "description": "返回当前收藏状态与 star 数。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "取消收藏",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "starred 与 stars",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/tags": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "列出标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/gitsvc.Tag"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/team-grants": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "仓库团队授权列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.RepoTeamGrant"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/team-grants/{teamId}": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "授予团队仓库权限",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "teamId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "permission",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "tags": [
                    "repos"
                ],
                "summary": "撤销团队仓库权限",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "团队 ID",
                        "name": "teamId",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/template": {
            "post": {
                "description": "仅仓库所有者可设置。模版仓库可作为创建新仓库的源。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置模版仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "is_template",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoTemplateReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/topics": {
            "put": {
                "description": "全量替换仓库的 topics；仅仓库所有者可操作。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置仓库标签",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "topics 列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoTopsReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/tree": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "浏览目录树",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者（简写路由时省略）",
                        "name": "owner",
                        "in": "path"
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "分支/标签/commit（默认默认分支）",
                        "name": "ref",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "目录路径（默认根目录）",
                        "name": "path",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "path、entries 与 truncated",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/visibility": {
            "post": {
                "description": "仅仓库所有者可设置。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "repos"
                ],
                "summary": "设置仓库可见性",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "private",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.setRepoVisibilityReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/store.Repo"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/watch": {
            "put": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "watch"
                ],
                "summary": "关注仓库",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "watching 与 watchers",
                        "schema": {
                            "type": "object"
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/webhooks": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "列出 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "创建 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "url、可选 secret（至少 16 字符）与可选 events（订阅的事件类型，空=全部）",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/api.createWebhookReq"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "type": "object"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/webhooks/{id}": {
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "删除 webhook",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "webhook ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{owner}/repos/{name}/webhooks/{id}/deliveries": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "列出 webhook 投递记录",
                "parameters": [
                    {
                        "type": "string",
                        "description": "仓库所有者",
                        "name": "owner",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "仓库名",
                        "name": "name",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "webhook ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "返回条数（默认 20，最大 100）",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "object"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}": {
            "get": {
                "description": "返回用户公开资料、followers/following 数量、当前用户是否已关注，以及其可见仓库（他人仅公开仓库）。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "用户主页",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}/avatar": {
            "get": {
                "description": "返回用户头像图片；未设置头像返回 404。",
                "produces": [
                    "image/png"
                ],
                "tags": [
                    "users"
                ],
                "summary": "用户头像",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}/cover": {
            "get": {
                "description": "返回用户主页顶部横幅图片；未设置封面返回 404。",
                "produces": [
                    "image/png"
                ],
                "tags": [
                    "users"
                ],
                "summary": "用户封面",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}/follow": {
            "post": {
                "description": "幂等；不能关注自己。返回关注状态与统计。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "关注用户",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            },
            "delete": {
                "description": "幂等。返回关注状态与统计。",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "取消关注",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": true
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}/followers": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "关注者列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.UserSummary"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/users/{username}/following": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "users"
                ],
                "summary": "关注列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "用户名",
                        "name": "username",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.UserSummary"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/watched": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "watch"
                ],
                "summary": "列出我关注的仓库",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "每页数量（默认 200，最大 500）",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/store.Repo"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        },
        "/webhook-events": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "webhooks"
                ],
                "summary": "列出 webhook 事件类型",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "type": "string"
                            }
                        }
                    }
                },
                "security": [
                    {
                        "BearerAuth": []
                    }
                ]
            }
        }
    },
    "definitions": {
        "api.addCollabReq": {
            "type": "object",
            "properties": {
                "permission": {
                    "description": "权限：read 或 write",
                    "type": "string"
                },
                "username": {
                    "description": "协作者用户名",
                    "type": "string"
                }
            }
        },
        "api.addCommentReq": {
            "type": "object",
            "properties": {
                "body": {
                    "description": "评论正文（最多 10000 字符）",
                    "type": "string"
                },
                "file_path": {
                    "description": "可选：行内评论的文件路径（仅 PR；填写时 line 必填，line_side 为 old/new）",
                    "type": "string"
                },
                "line": {
                    "type": "integer"
                },
                "line_side": {
                    "type": "string"
                }
            }
        },
        "api.addGPGKeyReq": {
            "type": "object",
            "properties": {
                "armor": {
                    "description": "armored 格式的 GPG 公钥",
                    "type": "string"
                }
            }
        },
        "api.addOrgMemberReq": {
            "type": "object",
            "properties": {
                "role": {
                    "description": "角色：member 或 owner，默认 member",
                    "type": "string"
                },
                "username": {
                    "description": "成员用户名",
                    "type": "string"
                }
            }
        },
        "api.adminChangePasswordReq": {
            "type": "object",
            "properties": {
                "current_password": {
                    "description": "当前密码",
                    "type": "string"
                },
                "new_password": {
                    "description": "新密码（至少 8 位，且包含小写字母/大写字母/数字/特殊字符中的至少 3 类）",
                    "type": "string"
                }
            }
        },
        "api.adminCreateUserReq": {
            "type": "object",
            "properties": {
                "email": {
                    "type": "string"
                },
                "password": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "api.adminLoginReq": {
            "type": "object",
            "properties": {
                "password": {
                    "description": "管理员密码",
                    "type": "string"
                },
                "username": {
                    "description": "管理员用户名",
                    "type": "string"
                }
            }
        },
        "api.adminResetPasswordReq": {
            "type": "object",
            "properties": {
                "password": {
                    "type": "string"
                }
            }
        },
        "api.banReq": {
            "type": "object",
            "properties": {
                "banned": {
                    "type": "boolean"
                }
            }
        },
        "api.batchImportReq": {
            "type": "object",
            "properties": {
                "namespace": {
                    "description": "可选：目标组织",
                    "type": "string"
                },
                "private": {
                    "description": "是否私有，默认 true",
                    "type": "boolean"
                },
                "provider": {
                    "description": "已绑定的 provider",
                    "type": "string"
                },
                "repos": {
                    "description": "远程仓库 full_name 列表",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "api.batchImportResult": {
            "type": "object",
            "properties": {
                "imported": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "skipped": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                }
            }
        },
        "api.boardResp": {
            "type": "object",
            "properties": {
                "cards": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ProjectCard"
                    }
                },
                "columns": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ProjectColumn"
                    }
                },
                "project": {
                    "$ref": "#/definitions/store.Project"
                },
                "swimlanes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ProjectSwimlane"
                    }
                }
            }
        },
        "api.byokReq": {
            "type": "object",
            "properties": {
                "api_key": {
                    "type": "string"
                },
                "base_url": {
                    "type": "string"
                },
                "id": {
                    "description": "ID 仅用于测试连接：api_key 留空时回退到已保存密钥。",
                    "type": "integer"
                },
                "model": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                }
            }
        },
        "api.changePasswordReq": {
            "type": "object",
            "properties": {
                "current_password": {
                    "description": "当前密码",
                    "type": "string"
                },
                "new_password": {
                    "description": "新密码（至少 8 位，且包含小写字母/大写字母/数字/特殊字符中的至少 3 类）",
                    "type": "string"
                }
            }
        },
        "api.commitResp": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "date": {
                    "type": "string"
                },
                "gpg_status": {
                    "description": "GPG 签名状态：verified | unknown_key | invalid（无签名字段缺省，与旧行为兼容）",
                    "type": "string"
                },
                "gpg_verified": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                },
                "parents": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "refs": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "sha": {
                    "type": "string"
                }
            }
        },
        "api.connectionView": {
            "type": "object",
            "properties": {
                "avatar_url": {
                    "type": "string"
                },
                "base_url": {
                    "type": "string"
                },
                "connected": {
                    "type": "boolean"
                },
                "enabled": {
                    "type": "boolean"
                },
                "label": {
                    "type": "string"
                },
                "login": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                }
            }
        },
        "api.copilotCreateReq": {
            "type": "object",
            "properties": {
                "byok_id": {
                    "type": "integer"
                },
                "issue_number": {
                    "type": "integer"
                },
                "prompt": {
                    "type": "string"
                }
            }
        },
        "api.createIssueReq": {
            "type": "object",
            "properties": {
                "body": {
                    "description": "Issue 正文（可选，最长 10000 字符）",
                    "type": "string"
                },
                "title": {
                    "description": "Issue 标题（必填，最长 200 字符）",
                    "type": "string"
                }
            }
        },
        "api.createKeyReq": {
            "type": "object",
            "properties": {
                "name": {
                    "description": "密钥名称",
                    "type": "string"
                },
                "public_key": {
                    "description": "SSH 公钥内容（OpenSSH 格式）",
                    "type": "string"
                }
            }
        },
        "api.createLabelReq": {
            "type": "object",
            "properties": {
                "color": {
                    "description": "标签颜色（6 位十六进制，可带 # 前缀）",
                    "type": "string"
                },
                "name": {
                    "description": "标签名称（必填，最长 50 字符）",
                    "type": "string"
                }
            }
        },
        "api.createMilestoneReq": {
            "type": "object",
            "properties": {
                "description": {
                    "description": "里程碑描述（可选）",
                    "type": "string"
                },
                "title": {
                    "description": "里程碑标题（必填）",
                    "type": "string"
                }
            }
        },
        "api.createOrgReq": {
            "type": "object",
            "properties": {
                "display": {
                    "description": "显示名",
                    "type": "string"
                },
                "name": {
                    "description": "组织名（2-32 字符，小写字母/数字/_/-，字母数字开头）",
                    "type": "string"
                }
            }
        },
        "api.createPATReq": {
            "type": "object",
            "properties": {
                "cidrs": {
                    "description": "来源 IP/CIDR 白名单（可选，IPv4/IPv6 或 CIDR；空 = 不限）",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "expires_at": {
                    "description": "过期时间 RFC3339 UTC（可选；空 = 永不过期）",
                    "type": "string"
                },
                "name": {
                    "description": "令牌名称（必填，\u003c=100 字符）",
                    "type": "string"
                },
                "scopes": {
                    "description": "授权范围（可选，默认 [\"repo\"]；仅接受 repo/inbox/keys）",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "api.createPipelineRunReq": {
            "type": "object",
            "properties": {
                "delay": {
                    "description": "可选：延迟执行时长（如 \"30s\"、\"5m\"；最大 24h），到期后自动开始",
                    "type": "string"
                },
                "file": {
                    "description": "可选：指定流水线文件（如 .gitdash/ci.yml）；为空且为多文件时触发全部",
                    "type": "string"
                },
                "inputs": {
                    "description": "可选（仅 dispatch）：注入为 INPUT_\u003cKEY\u003e 环境变量",
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "ref": {
                    "description": "可选：分支或 tag 短名，为空且无 sha 时用默认分支",
                    "type": "string"
                },
                "sha": {
                    "description": "可选：直接指定提交（分支名/tag/SHA 均可）",
                    "type": "string"
                }
            }
        },
        "api.createProjectCardReq": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "column_id": {
                    "type": "integer"
                },
                "due_date": {
                    "type": "string"
                },
                "issue_number": {
                    "type": "integer"
                },
                "note": {
                    "description": "兼容：等价于 title",
                    "type": "string"
                },
                "start_date": {
                    "type": "string"
                },
                "swimlane_id": {
                    "type": "integer"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "api.createProjectColumnReq": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                }
            }
        },
        "api.createProjectSwimlaneReq": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                }
            }
        },
        "api.createPullReq": {
            "type": "object",
            "properties": {
                "body": {
                    "description": "PR 正文（可选）",
                    "type": "string"
                },
                "source_branch": {
                    "description": "源分支（必填，可带 refs/heads/ 前缀）",
                    "type": "string"
                },
                "target_branch": {
                    "description": "目标分支（必填，可带 refs/heads/ 前缀）",
                    "type": "string"
                },
                "title": {
                    "description": "PR 标题（必填）",
                    "type": "string"
                }
            }
        },
        "api.createRefReq": {
            "type": "object",
            "properties": {
                "from": {
                    "description": "起点引用，可选，缺省 HEAD",
                    "type": "string"
                },
                "name": {
                    "description": "分支/标签名",
                    "type": "string"
                },
                "type": {
                    "description": "类型：branch 或 tag",
                    "type": "string"
                }
            }
        },
        "api.createReleaseReq": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "tag_name": {
                    "type": "string"
                }
            }
        },
        "api.createRepoReq": {
            "type": "object",
            "properties": {
                "description": {
                    "description": "仓库描述",
                    "type": "string"
                },
                "name": {
                    "description": "仓库名",
                    "type": "string"
                },
                "namespace": {
                    "description": "可选：组织名（成员可把仓库建到组织下）",
                    "type": "string"
                },
                "private": {
                    "description": "是否私有，默认 true",
                    "type": "boolean"
                },
                "template": {
                    "description": "模板：空 = 空仓库；\"readme\" = 默认模版（README.md + .gitdash.yml）",
                    "type": "string"
                },
                "template_name": {
                    "type": "string"
                },
                "template_owner": {
                    "description": "从模版仓库创建：指定源仓库（owner/name），克隆其内容与历史。",
                    "type": "string"
                }
            }
        },
        "api.createReviewReq": {
            "type": "object",
            "properties": {
                "body": {
                    "description": "review 说明",
                    "type": "string"
                },
                "commit_sha": {
                    "description": "可选：针对的提交 SHA",
                    "type": "string"
                },
                "state": {
                    "description": "approve | request_changes | comment",
                    "type": "string"
                }
            }
        },
        "api.createRunnerTokenReq": {
            "type": "object",
            "properties": {
                "org": {
                    "type": "string"
                },
                "scope": {
                    "type": "string"
                }
            }
        },
        "api.createWebhookReq": {
            "type": "object",
            "properties": {
                "secret": {
                    "description": "可选签名密钥，至少 16 字符",
                    "type": "string"
                },
                "url": {
                    "description": "回调地址，http(s) URL，最长 2048 字符",
                    "type": "string"
                }
            }
        },
        "api.deleteAccountReq": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "启用 MFA 时的二次验证码",
                    "type": "string"
                },
                "password": {
                    "description": "当前密码",
                    "type": "string"
                }
            }
        },
        "api.feedbackBodyReq": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "api.forgeRepo": {
            "type": "object",
            "properties": {
                "clone_url": {
                    "type": "string"
                },
                "default_branch": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "full_name": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "private": {
                    "type": "boolean"
                }
            }
        },
        "api.forkRepoReq": {
            "type": "object",
            "properties": {
                "name": {
                    "description": "目标仓库名，缺省用源仓库名",
                    "type": "string"
                },
                "namespace": {
                    "description": "可选：组织命名空间",
                    "type": "string"
                }
            }
        },
        "api.importRepoReq": {
            "type": "object",
            "properties": {
                "name": {
                    "description": "目标仓库名，可选，缺省从 URL 推断",
                    "type": "string"
                },
                "namespace": {
                    "description": "可选：组织命名空间",
                    "type": "string"
                },
                "private": {
                    "description": "是否私有，默认 true",
                    "type": "boolean"
                },
                "private_key": {
                    "description": "可选：拉取私有仓库用的 SSH 私钥",
                    "type": "string"
                },
                "url": {
                    "description": "外部仓库地址（http(s)/ssh/git）",
                    "type": "string"
                }
            }
        },
        "api.ipBanReq": {
            "type": "object",
            "properties": {
                "cidr": {
                    "type": "string"
                },
                "note": {
                    "type": "string"
                }
            }
        },
        "api.loginReq": {
            "type": "object",
            "properties": {
                "password": {
                    "description": "密码",
                    "type": "string"
                },
                "username": {
                    "description": "用户名",
                    "type": "string"
                }
            }
        },
        "api.mailInboundReq": {
            "type": "object",
            "properties": {
                "from": {
                    "type": "string"
                },
                "in_reply_to": {
                    "type": "string"
                },
                "message_id": {
                    "description": "以下为可选的邮件线程/幂等字段（MTA 适配层尽量回传）。",
                    "type": "string"
                },
                "references": {
                    "type": "string"
                },
                "subject": {
                    "type": "string"
                },
                "text": {
                    "type": "string"
                },
                "to": {
                    "type": "string"
                }
            }
        },
        "api.mergePullReq": {
            "type": "object",
            "properties": {
                "method": {
                    "description": "合并方式（fast-forward/merge/squash，空 = fast-forward）",
                    "type": "string"
                }
            }
        },
        "api.mfaActivateReq": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "认证器 TOTP 代码",
                    "type": "string"
                }
            }
        },
        "api.mfaDisableReq": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "认证器 TOTP 代码",
                    "type": "string"
                },
                "password": {
                    "description": "当前密码",
                    "type": "string"
                }
            }
        },
        "api.mfaVerifyReq": {
            "type": "object",
            "properties": {
                "code": {
                    "description": "认证器 TOTP 代码",
                    "type": "string"
                },
                "mfa_token": {
                    "description": "登录时返回的临时 MFA token",
                    "type": "string"
                }
            }
        },
        "api.registerReq": {
            "type": "object",
            "properties": {
                "password": {
                    "description": "密码（至少 8 位，且包含小写字母/大写字母/数字/特殊字符中的至少 3 类）",
                    "type": "string"
                },
                "username": {
                    "description": "用户名（4-32 位小写字母/数字/_/-，字母或数字开头）",
                    "type": "string"
                }
            }
        },
        "api.registerRunnerReq": {
            "type": "object",
            "properties": {
                "labels": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "mode": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "token": {
                    "type": "string"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "api.revertCommitReq": {
            "type": "object",
            "properties": {
                "branch": {
                    "description": "目标分支（必填，通常为当前查看的分支）",
                    "type": "string"
                },
                "message": {
                    "description": "提交信息，可选，缺省自动生成 \"Revert \u003c原始主题\u003e\"",
                    "type": "string"
                }
            }
        },
        "api.setBranchProtectionReq": {
            "type": "object",
            "properties": {
                "block_deletion": {
                    "type": "boolean"
                },
                "block_force_push": {
                    "type": "boolean"
                },
                "merge_queue": {
                    "type": "boolean"
                },
                "min_approvals": {
                    "type": "integer"
                },
                "require_ci": {
                    "type": "boolean"
                },
                "require_codeowners": {
                    "type": "boolean"
                }
            }
        },
        "api.setIssueLabelsReq": {
            "type": "object",
            "properties": {
                "label_ids": {
                    "description": "要设置的标签 ID 列表",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "api.setIssueMilestoneReq": {
            "type": "object",
            "properties": {
                "milestone_id": {
                    "description": "里程碑 ID（0 = 清除）",
                    "type": "integer"
                }
            }
        },
        "api.setMirrorReq": {
            "type": "object",
            "properties": {
                "private_key": {
                    "description": "可选：推送用的 SSH 私钥",
                    "type": "string"
                },
                "url": {
                    "description": "镜像目标地址",
                    "type": "string"
                }
            }
        },
        "api.setPipelineReq": {
            "type": "object",
            "properties": {
                "enabled": {
                    "description": "是否启用流水线",
                    "type": "boolean"
                }
            }
        },
        "api.setPullStateReq": {
            "type": "object",
            "properties": {
                "state": {
                    "description": "目标状态（open 或 closed）",
                    "type": "string"
                }
            }
        },
        "api.setRefNoteReq": {
            "type": "object",
            "properties": {
                "note": {
                    "description": "备注内容，空串表示清除备注",
                    "type": "string"
                }
            }
        },
        "api.setRepoDefaultBranchReq": {
            "type": "object",
            "properties": {
                "branch": {
                    "description": "默认分支名（必须已存在）",
                    "type": "string"
                }
            }
        },
        "api.setRepoDescriptionReq": {
            "type": "object",
            "properties": {
                "description": {
                    "description": "仓库描述（可为空，最长 500 字符）",
                    "type": "string"
                }
            }
        },
        "api.setRepoEnvVarReq": {
            "type": "object",
            "properties": {
                "key": {
                    "description": "环境变量名（[A-Za-z_][A-Za-z0-9_]*）",
                    "type": "string"
                },
                "value": {
                    "description": "环境变量值",
                    "type": "string"
                }
            }
        },
        "api.setRepoIssuesReq": {
            "type": "object",
            "properties": {
                "has_issues": {
                    "description": "是否启用 issue（必填）",
                    "type": "boolean"
                }
            }
        },
        "api.setRepoMemberRoleReq": {
            "type": "object",
            "properties": {
                "role": {
                    "description": "read/triage/write/maintain/admin；空表示继承组织默认",
                    "type": "string"
                }
            }
        },
        "api.setRepoSecretReq": {
            "type": "object",
            "properties": {
                "name": {
                    "description": "secret 名（[A-Za-z_][A-Za-z0-9_]*）",
                    "type": "string"
                },
                "value": {
                    "description": "secret 值",
                    "type": "string"
                }
            }
        },
        "api.setRepoTemplateReq": {
            "type": "object",
            "properties": {
                "is_template": {
                    "description": "是否为模版仓库（必填）",
                    "type": "boolean"
                }
            }
        },
        "api.setRepoTopsReq": {
            "type": "object",
            "properties": {
                "topics": {
                    "description": "小写字母/数字/连字符，最多 20 个",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "api.setRepoVisibilityReq": {
            "type": "object",
            "properties": {
                "private": {
                    "description": "兼容：是否私有",
                    "type": "boolean"
                },
                "visibility": {
                    "description": "private | public | anonymous",
                    "type": "string"
                }
            }
        },
        "api.testByokReq": {
            "type": "object",
            "properties": {
                "api_key": {
                    "type": "string"
                },
                "base_url": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "model": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                }
            }
        },
        "api.updateIssueReq": {
            "type": "object",
            "properties": {
                "body": {
                    "description": "新正文（可选）",
                    "type": "string"
                },
                "pinned": {
                    "description": "是否置顶（可选）",
                    "type": "boolean"
                },
                "state": {
                    "description": "新状态 open/closed（可选）",
                    "type": "string"
                },
                "title": {
                    "description": "新标题（可选）",
                    "type": "string"
                }
            }
        },
        "api.updateLabelReq": {
            "type": "object",
            "properties": {
                "color": {
                    "description": "新颜色（空则保持不变）",
                    "type": "string"
                },
                "name": {
                    "description": "新名称（空则保持不变）",
                    "type": "string"
                }
            }
        },
        "api.updateMilestoneReq": {
            "type": "object",
            "properties": {
                "description": {
                    "description": "新描述",
                    "type": "string"
                },
                "state": {
                    "description": "状态（open/closed，空则保持不变）",
                    "type": "string"
                },
                "title": {
                    "description": "新标题",
                    "type": "string"
                }
            }
        },
        "api.updateProfileReq": {
            "type": "object",
            "properties": {
                "bio": {
                    "description": "个人简介（可选）",
                    "type": "string"
                },
                "email": {
                    "description": "邮箱；空串表示清除",
                    "type": "string"
                },
                "notify_email": {
                    "description": "邮件通知开关（可选）",
                    "type": "boolean"
                }
            }
        },
        "api.updateProjectCardReq": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "column_id": {
                    "type": "integer"
                },
                "due_date": {
                    "type": "string"
                },
                "issue_number": {
                    "description": "IssueNumber 更换 / 解除关联的 issue（0 = 解除）；仅文本卡片或 issue 卡片均可。",
                    "type": "integer"
                },
                "note": {
                    "description": "兼容：等价于 title",
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                },
                "start_date": {
                    "type": "string"
                },
                "swimlane_id": {
                    "type": "integer"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "api.updateProjectColumnReq": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                }
            }
        },
        "api.updateProjectReq": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "api.updateProjectSwimlaneReq": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                }
            }
        },
        "api.writeCommitReq": {
            "type": "object",
            "properties": {
                "branch": {
                    "description": "目标分支，可选，默认 main",
                    "type": "string"
                },
                "changes": {
                    "description": "文件变更列表（create/update/delete/delete_tree/move）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/gitsvc.FileChange"
                    }
                },
                "message": {
                    "description": "提交信息",
                    "type": "string"
                }
            }
        },
        "copilot.ChatMessage": {
            "type": "object",
            "properties": {
                "role": {
                    "type": "string"
                },
                "text": {
                    "type": "string"
                },
                "tools": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/copilot.ToolEvent"
                    }
                }
            }
        },
        "copilot.ToolEvent": {
            "type": "object",
            "properties": {
                "input": {
                    "type": "string"
                },
                "is_error": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "result": {
                    "type": "string"
                }
            }
        },
        "gitsvc.Blame": {
            "type": "object",
            "properties": {
                "commits": {
                    "type": "object",
                    "additionalProperties": {
                        "$ref": "#/definitions/gitsvc.BlameCommit"
                    }
                },
                "lines": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/gitsvc.BlameLine"
                    }
                },
                "path": {
                    "type": "string"
                },
                "truncated": {
                    "type": "boolean"
                }
            }
        },
        "gitsvc.BlameCommit": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "date": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                },
                "sha": {
                    "type": "string"
                }
            }
        },
        "gitsvc.BlameLine": {
            "type": "object",
            "properties": {
                "commit": {
                    "type": "string"
                },
                "content": {
                    "type": "string"
                },
                "line": {
                    "type": "integer"
                }
            }
        },
        "gitsvc.Blob": {
            "type": "object",
            "properties": {
                "content": {
                    "type": "string"
                },
                "encoding": {
                    "description": "utf-8 | binary | truncated",
                    "type": "string"
                },
                "latest_commit": {
                    "description": "LatestCommit 该文件最近一次变更的提交（由 API 层填充，可选）",
                    "allOf": [
                        {
                            "$ref": "#/definitions/gitsvc.Commit"
                        }
                    ]
                },
                "path": {
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                }
            }
        },
        "gitsvc.Branch": {
            "type": "object",
            "properties": {
                "is_head": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "note": {
                    "description": "Note 分支备注（由 API 层从数据库合并填充，非 git 数据）。",
                    "type": "string"
                }
            }
        },
        "gitsvc.Commit": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "date": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                },
                "parents": {
                    "description": "父提交 sha（用于提交图）；根提交为空",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "refs": {
                    "description": "指向该提交的本地分支/标签（如 \"HEAD -\u003e main\"、\"tag: v1.0\"）",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "sha": {
                    "type": "string"
                }
            }
        },
        "gitsvc.FileChange": {
            "type": "object",
            "properties": {
                "action": {
                    "type": "string"
                },
                "content": {
                    "type": "string"
                },
                "from": {
                    "type": "string"
                },
                "path": {
                    "type": "string"
                }
            }
        },
        "gitsvc.GCResult": {
            "type": "object",
            "properties": {
                "after_bytes": {
                    "type": "integer"
                },
                "before_bytes": {
                    "type": "integer"
                },
                "freed_bytes": {
                    "type": "integer"
                }
            }
        },
        "gitsvc.SearchHit": {
            "type": "object",
            "properties": {
                "line": {
                    "type": "integer"
                },
                "path": {
                    "type": "string"
                },
                "text": {
                    "type": "string"
                }
            }
        },
        "gitsvc.Tag": {
            "type": "object",
            "properties": {
                "message": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "note": {
                    "description": "Note 标签备注（由 API 层从数据库合并填充，非 git 数据）。",
                    "type": "string"
                },
                "sha": {
                    "type": "string"
                }
            }
        },
        "pipeline.Graph": {
            "type": "object",
            "properties": {
                "edges": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/pipeline.GraphEdge"
                    }
                },
                "image": {
                    "type": "string"
                },
                "nodes": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/pipeline.GraphNode"
                    }
                }
            }
        },
        "pipeline.GraphEdge": {
            "type": "object",
            "properties": {
                "from": {
                    "type": "string"
                },
                "to": {
                    "type": "string"
                }
            }
        },
        "pipeline.GraphNode": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "string"
                },
                "kind": {
                    "description": "start | step | parallel | sub | end",
                    "type": "string"
                },
                "label": {
                    "type": "string"
                },
                "when": {
                    "type": "string"
                }
            }
        },
        "store.AccessEntry": {
            "type": "object",
            "properties": {
                "role": {
                    "type": "string"
                },
                "sources": {
                    "description": "owner | org_member | collaborator | team:\u003cname\u003e",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "subject": {
                    "type": "string"
                }
            }
        },
        "store.Badge": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "emoji": {
                    "type": "string"
                },
                "has_image": {
                    "type": "boolean"
                },
                "id": {
                    "type": "integer"
                },
                "image_updated_at": {
                    "type": "string"
                },
                "label": {
                    "type": "string"
                },
                "slug": {
                    "type": "string"
                }
            }
        },
        "store.BadgeGrant": {
            "type": "object",
            "properties": {
                "badge_id": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "kind": {
                    "type": "string"
                },
                "label": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                }
            }
        },
        "store.BranchProtection": {
            "type": "object",
            "properties": {
                "block_deletion": {
                    "description": "禁止删除该分支",
                    "type": "boolean"
                },
                "block_force_push": {
                    "description": "禁止非快进（force push）",
                    "type": "boolean"
                },
                "branch": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "merge_queue": {
                    "description": "合并队列：合并请求入队串行执行",
                    "type": "boolean"
                },
                "min_approvals": {
                    "description": "合并门禁：需要的最少 approve 数（0 = 不设门禁）",
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "require_ci": {
                    "description": "合并门禁：要求 head 的 CI 通过",
                    "type": "boolean"
                },
                "require_codeowners": {
                    "description": "合并门禁：要求变更文件的 CODEOWNERS 所有者批准",
                    "type": "boolean"
                }
            }
        },
        "store.ByokKey": {
            "type": "object",
            "properties": {
                "base_url": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "key_set": {
                    "type": "boolean"
                },
                "model": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "store.Collab": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "permission": {
                    "description": "\"read\" | \"write\"",
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "store.Comment": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "file_path": {
                    "description": "行内评论（仅 PR；FilePath 为 nil 表示普通评论）",
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "in_reply_to": {
                    "description": "InReplyTo 若本条评论来自邮件回复，记录所回复通知的 Message-ID。",
                    "type": "string"
                },
                "line": {
                    "type": "integer"
                },
                "line_side": {
                    "type": "string"
                },
                "message_id": {
                    "description": "MessageID 本条评论发出的通知邮件 Message-ID（邮件线程 / 入站去重）。",
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "suggestion_applied_sha": {
                    "description": "SuggestionAppliedSHA 非空表示该评论内的 suggestion 已被应用（对应提交 SHA）。",
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "store.CopilotSession": {
            "type": "object",
            "properties": {
                "branch": {
                    "type": "string"
                },
                "byok_id": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "string"
                },
                "error": {
                    "type": "string"
                },
                "head_sha": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "issue_number": {
                    "description": "IssueNumber 关联的 issue 编号（0 = 未关联）。",
                    "type": "integer"
                },
                "pr_number": {
                    "description": "PRNumber 自动开出的 PR 编号（0 = 尚未开）。",
                    "type": "integer"
                },
                "prompt": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "store.CreatedPAT": {
            "type": "object",
            "properties": {
                "cidrs": {
                    "description": "来源 IP/CIDR 白名单；空 = 不限",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "expires_at": {
                    "description": "RFC3339 UTC；空 = 永不过期",
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "last_used_at": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "scopes": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "token": {
                    "description": "明文 token，仅创建响应中出现一次",
                    "type": "string"
                }
            }
        },
        "store.DeployKey": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "fingerprint": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "permission": {
                    "description": "read | write",
                    "type": "string"
                },
                "public_key": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                }
            }
        },
        "store.ExportComment": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "file_path": {
                    "type": "string"
                },
                "kind": {
                    "type": "string"
                },
                "line": {
                    "type": "integer"
                },
                "line_side": {
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                }
            }
        },
        "store.ExportGPGKey": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "fingerprint": {
                    "type": "string"
                }
            }
        },
        "store.ExportIssue": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "state": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "store.ExportKey": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "fingerprint": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "public_key": {
                    "type": "string"
                }
            }
        },
        "store.ExportPAT": {
            "type": "object",
            "properties": {
                "cidrs": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "expires_at": {
                    "type": "string"
                },
                "last_used_at": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "scopes": {
                    "type": "string"
                }
            }
        },
        "store.ExportProfile": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "email_verified": {
                    "type": "boolean"
                },
                "mfa_enabled": {
                    "type": "boolean"
                },
                "notify_email": {
                    "type": "boolean"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "store.ExportPull": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "source_branch": {
                    "type": "string"
                },
                "state": {
                    "type": "string"
                },
                "target_branch": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "store.ExportRelease": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "tag_name": {
                    "type": "string"
                }
            }
        },
        "store.ExportRepo": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "is_template": {
                    "type": "boolean"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "private": {
                    "type": "boolean"
                }
            }
        },
        "store.ExportReview": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "commit_sha": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "state": {
                    "type": "string"
                }
            }
        },
        "store.GPGKey": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "fingerprint": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                }
            }
        },
        "store.IPBan": {
            "type": "object",
            "properties": {
                "cidr": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "note": {
                    "type": "string"
                }
            }
        },
        "store.Issue": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "body": {
                    "type": "string"
                },
                "closed_at": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "number": {
                    "type": "integer"
                },
                "pinned": {
                    "description": "置顶",
                    "type": "boolean"
                },
                "state": {
                    "description": "\"open\" | \"closed\"",
                    "type": "string"
                },
                "state_reason": {
                    "description": "StateReason 关闭原因：completed | not_planned（仅 closed 时非空）",
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "store.IssueEvent": {
            "type": "object",
            "properties": {
                "action": {
                    "description": "opened | closed | reopened | edited | labeled | unlabeled | milestoned | demilestoned | pinned | unpinned | assigned | unassigned | commented",
                    "type": "string"
                },
                "actor": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "detail": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "kind": {
                    "description": "issue | pull",
                    "type": "string"
                },
                "number": {
                    "description": "宿主编号",
                    "type": "integer"
                }
            }
        },
        "store.Label": {
            "type": "object",
            "properties": {
                "color": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                }
            }
        },
        "store.LanguageStat": {
            "type": "object",
            "properties": {
                "bytes": {
                    "type": "integer"
                },
                "language": {
                    "type": "string"
                },
                "percent": {
                    "type": "number"
                }
            }
        },
        "store.Milestone": {
            "type": "object",
            "properties": {
                "closed_issues": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "open_issues": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "state": {
                    "description": "open | closed",
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "store.Notification": {
            "type": "object",
            "properties": {
                "action": {
                    "description": "opened | closed | reopened | merged",
                    "type": "string"
                },
                "actor": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "kind": {
                    "description": "issue | pull",
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "read": {
                    "type": "boolean"
                },
                "repo": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "store.Org": {
            "type": "object",
            "properties": {
                "banned": {
                    "type": "boolean"
                },
                "bio": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "default_member_role": {
                    "type": "string"
                },
                "display": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "store.OrgTeam": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "member_count": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "org": {
                    "type": "string"
                }
            }
        },
        "store.PAT": {
            "type": "object",
            "properties": {
                "cidrs": {
                    "description": "来源 IP/CIDR 白名单；空 = 不限",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "created_at": {
                    "type": "string"
                },
                "expires_at": {
                    "description": "RFC3339 UTC；空 = 永不过期",
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "last_used_at": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "scopes": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "store.Package": {
            "type": "object",
            "properties": {
                "checksum": {
                    "description": "sha256 hex",
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "downloads": {
                    "type": "integer"
                },
                "filename": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "meta": {
                    "description": "生态相关扩展元数据（JSON/文本），如 cargo deps/features、pypi Requires-Python",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "private": {
                    "description": "兼容字段：visibility == \"private\"",
                    "type": "boolean"
                },
                "repo": {
                    "description": "可选：关联仓库（跟随其可见性），空 = 不关联",
                    "type": "string"
                },
                "size": {
                    "type": "integer"
                },
                "type": {
                    "description": "npm | composer | pypi | rubygems | go | cargo | maven",
                    "type": "string"
                },
                "uploader": {
                    "type": "string"
                },
                "version": {
                    "type": "string"
                },
                "visibility": {
                    "description": "private | public | anonymous（匿名可读）",
                    "type": "string"
                },
                "yanked": {
                    "type": "boolean"
                }
            }
        },
        "store.PackageAudit": {
            "type": "object",
            "properties": {
                "action": {
                    "description": "publish | delete | yank | unyank",
                    "type": "string"
                },
                "actor": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                },
                "version": {
                    "type": "string"
                }
            }
        },
        "store.PipelineCIStatus": {
            "type": "object",
            "properties": {
                "run_id": {
                    "type": "integer"
                },
                "status": {
                    "description": "pending | running | success | failed",
                    "type": "string"
                }
            }
        },
        "store.Project": {
            "type": "object",
            "properties": {
                "card_count": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                }
            }
        },
        "store.ProjectCard": {
            "type": "object",
            "properties": {
                "assignees": {
                    "description": "Assignees 负责人用户名；Labels 复用的仓库标签。",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "body": {
                    "description": "Markdown 详情",
                    "type": "string"
                },
                "column_id": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "due_date": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "issue_number": {
                    "type": "integer"
                },
                "issue_state": {
                    "type": "string"
                },
                "issue_title": {
                    "type": "string"
                },
                "labels": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.Label"
                    }
                },
                "note": {
                    "description": "兼容字段：与 title 同步",
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                },
                "project_id": {
                    "type": "integer"
                },
                "start_date": {
                    "type": "string"
                },
                "swimlane_id": {
                    "description": "0 = 未分组泳道",
                    "type": "integer"
                },
                "title": {
                    "type": "string"
                }
            }
        },
        "store.ProjectColumn": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                },
                "project_id": {
                    "type": "integer"
                }
            }
        },
        "store.ProjectSwimlane": {
            "type": "object",
            "properties": {
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "position": {
                    "type": "integer"
                },
                "project_id": {
                    "type": "integer"
                }
            }
        },
        "store.PullRequest": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "auto_merge": {
                    "description": "AutoMerge 开启后，合并门禁满足时由服务端自动合并。",
                    "type": "boolean"
                },
                "auto_merge_method": {
                    "type": "string"
                },
                "base_sha": {
                    "type": "string"
                },
                "body": {
                    "type": "string"
                },
                "ci": {
                    "description": "head 提交的流水线状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/store.PipelineCIStatus"
                        }
                    ]
                },
                "conflicted": {
                    "description": "分支分叉且存在合并冲突",
                    "type": "boolean"
                },
                "created_at": {
                    "type": "string"
                },
                "draft": {
                    "type": "boolean"
                },
                "head_sha": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "merge_queued": {
                    "description": "已加入合并队列",
                    "type": "boolean"
                },
                "mergeable": {
                    "description": "API enrich（仅 open PR / 详情接口填充）",
                    "type": "boolean"
                },
                "merged_at": {
                    "type": "string"
                },
                "merged_by": {
                    "type": "string"
                },
                "number": {
                    "type": "integer"
                },
                "source_branch": {
                    "type": "string"
                },
                "state": {
                    "description": "open | merged | closed",
                    "type": "string"
                },
                "target_branch": {
                    "type": "string"
                },
                "title": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "store.PullReview": {
            "type": "object",
            "properties": {
                "body": {
                    "type": "string"
                },
                "commit_sha": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "number": {
                    "type": "integer"
                },
                "reviewer": {
                    "type": "string"
                },
                "state": {
                    "type": "string"
                }
            }
        },
        "store.Quota": {
            "type": "object",
            "properties": {
                "max_gpg_keys_per_user": {
                    "type": "integer"
                },
                "max_org_members": {
                    "type": "integer"
                },
                "max_orgs_per_user": {
                    "type": "integer"
                },
                "max_pats_per_user": {
                    "type": "integer"
                },
                "max_repos_per_org": {
                    "type": "integer"
                },
                "max_repos_per_user": {
                    "type": "integer"
                },
                "max_ssh_keys_per_user": {
                    "type": "integer"
                },
                "max_webhooks_per_repo": {
                    "type": "integer"
                }
            }
        },
        "store.Release": {
            "type": "object",
            "properties": {
                "author": {
                    "type": "string"
                },
                "body": {
                    "description": "markdown",
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "repo": {
                    "type": "string"
                },
                "tag_name": {
                    "type": "string"
                }
            }
        },
        "store.ReleaseAsset": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "filename": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "release_id": {
                    "type": "integer"
                },
                "size": {
                    "type": "integer"
                }
            }
        },
        "store.Repo": {
            "type": "object",
            "properties": {
                "banned": {
                    "type": "boolean"
                },
                "created_at": {
                    "type": "string"
                },
                "default_branch": {
                    "description": "DefaultBranch 仓库默认分支（git HEAD）；旧数据为空时视为 main。",
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "fork_owner": {
                    "type": "string"
                },
                "fork_repo": {
                    "type": "string"
                },
                "has_issues": {
                    "description": "HasIssues 是否启用 issue 功能。",
                    "type": "boolean"
                },
                "id": {
                    "type": "integer"
                },
                "import_error": {
                    "type": "string"
                },
                "import_status": {
                    "description": "导入任务状态（queued/running/synced/failed），非导入仓库为空",
                    "type": "string"
                },
                "import_url": {
                    "type": "string"
                },
                "is_org": {
                    "description": "IsOrg 仓库归属组织（仅用于设置界面判断，由 API 层填充）。",
                    "type": "boolean"
                },
                "is_template": {
                    "type": "boolean"
                },
                "language": {
                    "description": "Language 主要语言（列表页展示，由 API 层批量填充）",
                    "type": "string"
                },
                "languages": {
                    "description": "Languages 语言构成（详情/code 页展示 top N，由 API 层填充）",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.LanguageStat"
                    }
                },
                "member_role": {
                    "description": "MemberRole 组织成员在本仓库的默认角色覆盖（空 = 继承组织默认）。",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "pages_branch": {
                    "type": "string"
                },
                "pages_dir": {
                    "type": "string"
                },
                "pages_enabled": {
                    "description": "Pages 静态网站托管（默认关闭）。",
                    "type": "boolean"
                },
                "private": {
                    "type": "boolean"
                },
                "role": {
                    "description": "Role 仅用于“可访问仓库列表”（owner / read / write），普通查询为空",
                    "type": "string"
                },
                "size": {
                    "description": "仓库磁盘占用（字节），由 API 层按需填充",
                    "type": "integer"
                },
                "starred": {
                    "type": "boolean"
                },
                "stars": {
                    "description": "展示字段（由 API 层填充，store 查询不扫描）",
                    "type": "integer"
                },
                "topics": {
                    "description": "Topics 仓库标签/话题（由 API 层批量填充，store 查询不扫描）",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "visibility": {
                    "description": "private | public | anonymous（匿名只读）",
                    "type": "string"
                },
                "watchers": {
                    "type": "integer"
                },
                "watching": {
                    "type": "boolean"
                }
            }
        },
        "store.RepoRef": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                }
            }
        },
        "store.RepoTeamGrant": {
            "type": "object",
            "properties": {
                "permission": {
                    "type": "string"
                },
                "team_id": {
                    "type": "integer"
                },
                "team_name": {
                    "type": "string"
                }
            }
        },
        "store.Runner": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "labels": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "last_seen": {
                    "type": "string"
                },
                "mode": {
                    "description": "\"\"=agent 主动外连（dial） | \"reverse\"=服务端主动拨号",
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "scope": {
                    "description": "\"\"=全局 | \"user:{owner}\" | \"org:{org}\"",
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "url": {
                    "type": "string"
                }
            }
        },
        "store.RunnerToken": {
            "type": "object",
            "properties": {
                "expires_at": {
                    "type": "string"
                },
                "scope": {
                    "type": "string"
                }
            }
        },
        "store.SSHKey": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "fingerprint": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "public_key": {
                    "type": "string"
                }
            }
        },
        "store.TopicCount": {
            "type": "object",
            "properties": {
                "count": {
                    "type": "integer"
                },
                "topic": {
                    "type": "string"
                }
            }
        },
        "store.User": {
            "type": "object",
            "properties": {
                "banned": {
                    "type": "boolean"
                },
                "bio": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "username": {
                    "type": "string"
                }
            }
        },
        "store.UserExport": {
            "type": "object",
            "properties": {
                "comments": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportComment"
                    }
                },
                "generated_at": {
                    "type": "string"
                },
                "gpg_keys": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportGPGKey"
                    }
                },
                "issues": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportIssue"
                    }
                },
                "pats": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportPAT"
                    }
                },
                "profile": {
                    "$ref": "#/definitions/store.ExportProfile"
                },
                "pulls": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportPull"
                    }
                },
                "releases": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportRelease"
                    }
                },
                "repos": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportRepo"
                    }
                },
                "reviews": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportReview"
                    }
                },
                "ssh_keys": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.ExportKey"
                    }
                },
                "stars": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.RepoRef"
                    }
                },
                "watches": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/store.RepoRef"
                    }
                }
            }
        },
        "store.UserSummary": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "username": {
                    "type": "string"
                }
            }
        }
    },
    "securityDefinitions": {
        "BearerAuth": {
            "description": "形如 \"Bearer \u003ctoken\u003e\"。",
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "",
	BasePath:         "",
	Schemes:          []string{},
	Title:            "gitdash API",
	Description:      "git 托管服务 REST API。除 /api/health、/api/version、/api/openapi.json 与认证端点外，均需 Bearer token（由注册 / 登录返回）。",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
